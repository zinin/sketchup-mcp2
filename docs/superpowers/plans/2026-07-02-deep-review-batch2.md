...
- **Ruby-тесты:** каждый `test/test_*.rb` обязан проходить (a) standalone `ruby test/test_<name>.rb` и (b) в одном процессе `ruby test/run_all.rb`. Глобальные стабы (`module Sketchup`, `module Geom`) — только guarded (`unless defined?(...)`); singleton-поверхности патчить в `setup`/`teardown` с сохранением `Method`-объекта (эталон: `test/test_collect_components.rb`, патч `Helpers::Entities.active_model!`).
- **Прогоны и базлайны на старте батча:** Ruby — `ruby test/run_all.rb` → **354 runs / 939 assertions / 0 failures / 0 errors** (~1.4 с); Python — `uv run pytest tests/ -q` → **136 passed** (~2.4 с).
...
def test_unknown_log_level_warns_and_falls_back_to_info(env_clean, monkeypatch, caplog):
    ...
    assert any("SKETCHUP_MCP_LOG_LEVEL" in r.message % r.args for r in caplog.records)
...
⚠ В `caplog`-ассерте `r.message % r.args` — потому что warning ниже эмитится ленивым `%`-форматированием (`logger.warning("%s: ...", name, raw)`).

- [ ] **Step 2: Прогнать — убедиться, что RED соответствует прогнозу**

Run: `uv run pytest tests/test_config.py -q`
Expected: **10 failed, 1 passed** (+6 старых passed = «6 passed» строкой выше не появится — итог `10 failed, 7 passed`). Провалы: port «abc» — ValueError без имени переменной (match не находит); port «0»/«65536»/«-5» и timeout «0»/«-1»/«inf»/«nan» — DID NOT RAISE; timeout «abc» — ValueError без имени; VERBOSE — LOG_LEVEL == "VERBOSE" ≠ "INFO". `test_warn_level_accepted` уже зелёный (пин текущего поведения).
...
Run: `uv run pytest tests/test_config.py -q`
Expected: **17 passed** (6 старых + 11 новых).
...
Run: `uv run pytest tests/ -q`
Expected: **153 passed** (151 − 4 удалённых + 6 новых; версионный swap ±0). 0 failed.
...
Expected: 3 новых FAIL (int-id ловит ValidationError на строгом `str`), `test_empty_string_id_still_rejected` — PASS (пин текущего поведения).
...
Run: `uv run pytest tests/ -q`
Expected: **157 passed** (153 + 4). Существующая параметризация `test_tool_wrapper_calls_ruby_correctly` передаёт id строками — проходит без правок (str→str неизменен).
...
Run: `uv run pytest tests/ -q`
Expected: **161 passed** (157 + 4). Wire-pin строка create_component в `test_tool_wrapper_calls_ruby_correctly` передаёт dimensions явно — не задета.
...
**Interfaces:**
- Produces: ответ `list_components`/`find_components` — `{"components": [...], "total": N, "offset": M, "truncated": bool}`; параметры `limit` (1..500, дефолт 50), `offset` (≥0, дефолт 0), `response_format` (`"concise"|"detailed"`, дефолт `"detailed"`; concise режет каждый элемент до `{id, name, type, depth}`). `V.optional_int_nonneg(params, key, default)`, `V.optional_enum(params, key, allowed, default)`. `Model.find_component_by_id(entities, target_id, parent_t, depth:, max_depth:, seen:)` — DFS с ранним выходом, возвращает describe-хеш или nil.
...
3a. Константа рядом с `DEFAULT_MAX_DEPTH`:
...
```ruby
      ...
      def self.pagination_params(params)
        [
          V.optional_int_positive(params, "limit", DEFAULT_LIMIT),
...
Комментарий над методом обновить: убрать слова «Reuses collect_components» → «Uses find_component_by_id (early-exit DFS, same world-frame math as collect_components)»; остальной текст (fallback, LOOKUP_MAX_DEPTH) остаётся верным.
...
Run: `uv run pytest tests/ -q`
Expected: **164 passed** (161 + 3; правки таблицы счётчик не меняют).
...
**Interfaces:**
- Produces: везде, где наружу уходит `bounding_box_mm`/`bbox_mm`, пустые bounds (инвертированный сентинел SketchUp: `min > max`, ±1e30 дюймов) дают `null` вместо ±2.54e31. Хелпер `Model.bbox_mm_or_nil(bb)`.
- Consumes: стабы Sketchup/Geom (паттерн Task 7).
- ⚠ Форма ответа: потребители из Task 15 (докстринги: «bbox_mm may be null»), Task 17 (smoke не задет — там непустые entity).
...
- [ ] **Step 2: GREEN — `handlers/model.rb`**

2a. Хелпер (после `describe_component`):

```ruby
      # T-55: пустой Geom::BoundingBox SketchUp — «инвертированный» сентинел
      # (min = +1e30 дюймов, max = −1e30). Наружу он утекал как ±2.54e31 мм
      # и выглядел валидными координатами. Отдаём null.
      def self.bbox_mm_or_nil(bb)
        return nil if bb.min.x > bb.max.x
        {
...
2c. `describe_component`: сразу после `bb = entity.bounds` вставить:

```ruby
        if bb.min.x > bb.max.x  # T-55: пустые bounds — см. bbox_mm_or_nil
          return {
            "id"    => entity.entityID,
            "name"  => entity.name,
...
⚠ `test/test_transform_absolute.rb` — literal source-guard: прогнать его standalone; если он пинит текст `describe_entity` — обновить пин осознанно, зафиксировав в commit message.

- [ ] **Step 4: Прогнать + commit**

```bash
ruby test/test_model_empty_bbox.rb    # 3 runs, 0 failures
ruby test/test_transform_absolute.rb  # standalone: пины живы (или обновлены осознанно)
ruby test/run_all.rb                  # 370 runs (367+3), 0 failures
uv run pytest tests/ -q               # 164 passed (Python не задет)
...
Аннотацию возврата функции сменить с `-> Image:` на `-> list[Image | str]:`. Если FastMCP на регистрации схемы споткнётся об эту аннотацию (важно: `uv run pytest tests/ -q` упадёт на импорте немедленно) — fallback: `-> list:` (unstructured content path); зафиксировать выбор в commit message.
...
Run → оба FAIL (константы не определены → NameError; поведение отсутствует).
...
```ruby
      ...
      FRAME_QUEUE_SOFT_MAX      = 256     # backpressure: очередь забита — тик не читает сокеты (T-13.2)
```
...
Run: standalone + run_all → 0 failures. Существующий тест overflow-guard'а (ищи `pending_write_overflow` в test_server_multi_client.rb) обязан остаться зелёным: он набивает хвост при живом head — семантика сохранена; если он предполагал старую формулу (без вычета head) — обновить его ассерты осознанно, зафиксировав в commit message.
...
Run → FAIL (`connected_at` не определён).
...
```ruby
      ...
      # exposed-bind). Wire-протокол не меняется — это чисто серверный таймер.
...
1a. RED — в `test/test_compat.rb::test_too_new_raises_with_reinstall_hint` добавить ассерты (после `assert_includes err.message, ".rbz"`):
...
⚠ Под run_all `Validation`/`Entities` — реальные модули: `define_singleton_method` затенит настоящие методы. Здесь допустимо ТОЛЬКО потому, что стаб снимается `remove_method` в ensure — singleton-метод удаляется, и резолвинг возвращается к реальному методу модуля. Это отличается от паттерна «сохранить Method» (test_collect_components.rb) — тот нужен, когда патчится сам модульный метод; здесь мы добавляем НОВЫЙ singleton поверх и удаляем его же.
...
```ruby
      # T-16: definition.entities у ComponentInstance (и у скопированных
      # Group) ШАРИТСЯ между инстансами — мутация через entity_collection
      # красит/режет все копии разом («четыре стула краснеют одним
      # set_material»). Мутирующие хендлеры обязаны ходить сюда: make_unique
      # отвязывает entity в собственную definition (для уже-уникального —
      # дёшево, для объекта без make_unique — no-op). Read-only обходы
      # (list/find/get_component_info) остаются на entity_collection.
...
- `handlers/operations.rb::run_edge_op`: `target_entities = E.entity_collection(entity)` → `target_entities = E.mutable_entity_collection(entity)` (chamfer/fillet перестраивают рёбра внутри entity — мутация; строка `cur_edges = E.entity_collection(entity)...` ниже остаётся: она читает уже-уникальный после первого вызова entity).

⚠ Проверить пины: `ruby test/test_joints_frame_compensation.rb` standalone — FakeBoard не имеет `make_unique`, guard `respond_to?` пропускает; source-пины этого файла текст `place_tenon`/`add_parent_frame_prototype` НЕ пинят построчно (только carve_*-роутинг и sibling-cutter) — но при падении пина обновлять осознанно.
...
class TestJointsValidation < Minitest::Test
  ...
  def test_dovetail_angle_at_60_passes_validation
    # 60° — граница включительно; дальше валидации тест не идёт: E-стаб пуст,
    # NoMethodError от active_model! означает «валидация пройдена».
...
Run: `uv run pytest tests/ -q`
Expected: **168 passed** (165 + 3). ⚠ Wire-pin таблица не задета: строка create_component передаёт `[1, 1, 1]` — ровно на floor'е (≥ 1.0 ✓).
...
```ruby
  ...
  def test_handler_script_error_arm_produces_minus_32603
    # Ре-верификация «мёртвого» arm'а (финальное ревью батча 1, qwen —
    # опровергнуто): ScriptError не StandardError и без arm'а молча ронял
    # бы ответ (клиент ждал бы полный таймаут).
    sys = MCPforSketchUp::Handlers::System
    original = sys.method(:get_version)
    sys.define_singleton_method(:get_version) { |_params| raise ScriptError, "not loadable" }
    begin
      resp = MCPforSketchUp::Handlers::Dispatch.handle(
        "jsonrpc" => "2.0", "method" => "tools/call",
        "params" => { "name" => "get_version", "arguments" => {} }, "id" => 8)
      assert_equal(-32603, resp["error"]["code"])
      assert_match(/ScriptError/, resp["error"]["message"])
      assert_equal 8, resp["id"]
    ensure
      sys.define_singleton_method(:get_version, original)
    end
  end
...
git commit -m "test: cover dispatch error paths — StandardError and ScriptError arms (T-23)"
```
...
class TestJointsFrameCompensationRotated < Minitest::Test
  ...
  # мировой bbox (родительский фрейм) x 30..34, y 0..4.
...
Проверка дискриминативности (обязательна): в `world_points` временно заменить `acc.compose(inst[:transformation])` на `acc` (игнор компенсации) — оба теста обязаны УПАСТЬ; вернуть обратно.
...
async def test_dimension_tools_mention_units():
    ...
    for name in ("create_component", "transform_component", "chamfer_edge",
                 ...
                 "create_finger_joint", "export_scene"):
...
Run: `uv run pytest tests/test_tool_descriptions.py -q` → 4 FAIL (описаний нет; заметки «Ruby tool name» / «Pydantic» на месте; цвета не перечислены).
...
- Geometry, material, boolean, and edge tools return
  {id, name, type, bbox_mm} (edge tools add edges_*/stats). bbox_mm may be
  null for empty geometry. When bbox_mm is present, read it to confirm the
  result matches the intent before the next step (and to relocate the
  entity if its id becomes stale after destructive operations).
...
3c. §3: добавить пункт `- create_component minimum dimension is 1.0 mm; defaults are a 100 mm cube.`

- [ ] **Step 4: Прогнать + commit**

Run: `uv run pytest tests/ -q`
Expected: **172 passed** (168 + 4). ⚠ `tests/test_prompts.py` пинит фрагменты стратегии — если пины упали, обновить их под новый текст ОСОЗНАННО (в том же коммите). Wire-пины (`test_tool_wrapper_calls_ruby_correctly`) обязаны пройти без правок — поведение не менялось.
...
- `README.md:137-138`: удалить строки про `arts_and_crafts_cabinet.py` и `simple_test.py, simple_ruby_eval.py, behavior_tester.py` (git log подтверждает: никогда не коммитились). Вместо них:

```markdown
- `smoke_check.py` — 25-step end-to-end smoke against a live SketchUp.
- `smoke_multi_client.py` — concurrent-clients load check.
```
...
```bash
uv run pytest tests/ -q     # 172 passed — pyproject-правка не ломает метаданных
...
```bash
ruby test/run_all.rb        # ориентир: ~411 runs (354 на старте + ~57 новых), 0 failures
uv run pytest tests/ -q     # ориентир: 172 passed
```
