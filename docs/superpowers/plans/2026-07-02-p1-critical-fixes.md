...
```toml
dependencies = [
    # <2: mcp v2 ships breaking changes to the import surface. Floor 1.27 =
    # фактическая версия из uv.lock. Миграция на v2 — отдельный тикет.
    "mcp[cli]>=1.27,<2"
]
```
...
- Produces: workflow `test` с job'ами `ruby` и `python`. Триггеры: `pull_request` (первый реальный прогон ветки случится при открытии PR) и `push` в master. Коммиты в `fix/deep-review-p1` ДО открытия PR CI НЕ гоняют — «под CI» задачи 3–10 окажутся только на PR; локальные прогоны сьют остаются основной верификацией внутри плана.
...
jobs:
  ...
  python:
    ...
    strategy:
      fail-fast: false
      matrix:
        # 3.10 — заявленный requires-python floor: CI обязан тестировать минимум.
    ...
    steps:
      ...
      - uses: astral-sh/setup-uv@v5
        with:
          ...
          enable-cache: true
      # --frozen: lock обязан соответствовать pyproject (задача 1 его регенерирует);
      # рассинхрон = красный CI, а не тихая пересборка резолвером.
...
Дополнительно проверить floor локально: `uv run --python 3.10 pytest tests/ -q` (uv скачает интерпретатор при необходимости). Если на 3.10 сьюта падает — это реальная несовместимость с заявленным `requires-python >= 3.10`: остановиться и обсудить с владельцем (поднимать floor или чинить), НЕ выкидывать 3.10 из матрицы молча.
...
NB исполнителю: `require_relative .../helpers/geometry` в списке выше нужен, только если цепочка загрузки `handlers/operations` его требует — проверить; если файл грузится и тесты зелёные без него, строку убрать.
...
- [ ] **Step 5: Верифицировать, что guard'ы реально guard'ят (адверсариальная проверка)**

⚠ minitest исполняет только ПЕРВЫЙ файл из CLI-аргументов (второе имя уходит в ARGV и молча игнорируется) — файлы прогонять ОТДЕЛЬНЫМИ командами. Откатывать флипы ТОЧЕЧНОЙ правкой строки, НЕ `git checkout -- <файл>` (снесёт другие незакоммиченные правки файла).

1. Временно флипнуть `operations.rb:49` на `target_copy.subtract(tool_copy)`:
   - `ruby test/test_boolean_direction.rb` — обязан упасть поведенческий тест;
   - `ruby test/test_operation_names.rb` — обязан упасть `test_boolean_difference_receiver_is_tool_copy`.
   Вернуть строку (`tool_copy.subtract(target_copy)`), перепрогнать оба файла — зелено.
2. Временно флипнуть в `joints.rb` call-site `subtract_tracked(cutter, pin_group)` → `subtract_tracked(pin_group, cutter)`: `ruby test/test_operation_names.rb` — обязан упасть `test_joints_subtract_call_sites_keep_cutter_first`. Вернуть, перепрогнать — зелено.
3. Временно флипнуть в теле `subtract_tracked` строку `result = cutter.subtract(target)` → `result = target.subtract(cutter)`: `ruby test/test_operation_names.rb` — обязан упасть `test_subtract_tracked_body_receiver_is_cutter`. Вернуть, перепрогнать — зелено.
...
- [ ] **Step 3b: Нижняя граница segments (попутная робастность)**

В начало `build_sphere` добавить валидацию: при `segments < 3` — `raise MCPforSketchUp::Core::StructuredError.new(-32602, "segments must be >= 3 for spheres")` (segments 1–2 молча давали вырожденную геометрию). В `test_geometry_builders.rb` добавить тест: `assert_raises(MCPforSketchUp::Core::StructuredError) { GEO.build_sphere(FakeEntities.new, [0.0, 0.0, 0.0], [4.0, 4.0, 4.0], 2) }`.
...
Адверсариальная калибровка оракула: временно пропустить эмиссию одной полюсной грани (`next if lat_i == 0 && lon_i == 0` перед `begin`) — обязаны упасть и счётчик граней, и `test_sphere_face_mesh_is_manifold`; вернуть, перепрогнать — зелено. NB: edge-count через `[a, b].sort` нечувствителен к winding'у — инверсию ПОРЯДКА вершин грани оракул не ловит (только удаление/дубль ребра); осознанная граница.
...
- [ ] **Step 4b: Поведенческий fake-тест хендлера transform_component (дополнение по ревью)**

Position-путь хендлера (mm→inch на границе + дельта от `bounds.min` + `transform!`) до live-smoke закрыт только pure-хелпером и source-guard'ом — для breaking change этого мало. В `test/test_transform_absolute.rb` добавить отдельный тест-класс по образцу фейков `test_boolean_direction.rb` + runtime-патча Geom из `test_helpers_geometry.rb`:

- фейки: model (`start_operation`/`commit_operation`/`abort_operation`, `find_entity_by_id`), entity с `bounds.min = (10, 0, 0)` дюймов, записывающий вызовы `transform!(t)`; `Geom::Transformation.translation` / `Geom::Point3d` — стаб/патч, возвращающий маркер с вектором (в setup/teardown с восстановлением, конвенция сьюты);
- вызов: `GEO.transform_component("id" => <id>, "position" => [381.0, 0.0, 0.0])` — 381 мм = ровно 15 дюймов;
- ассерты: `transform!` вызван ровно один раз; его translation-вектор равен `(5.0, 0.0, 0.0)` (15 − 10 дюймов); rotation/scale-веток не было.

Если фактические сигнатуры/хелперы хендлера отличаются от ожиданий — адаптировать фейки, сохранив смысл ассертов (конвертация mm→inch на границе + дельта от bbox-min). Прогнать: `ruby test/test_transform_absolute.rb && ruby test/run_all.rb` — PASS.
...
- Produces: новый приватный хелпер `Joints.add_parent_frame_prototype(board) { |prot| ... }`, инкапсулирующий паттерн; три carve-хелпера строят геометрию через него. Механика бага: хелперы клали геометрию в board-ЛОКАЛЬНУЮ коллекцию (`E.entity_collection(board)`), считая координаты от `board.bounds.center` (РОДИТЕЛЬСКИЙ фрейм) → двойное смещение на величину трансформации доски; живьём: доска `x 800..920`, хвосты улетели в `x 800..1704`, `boolean_cuts.failed=0` рапортовал ложный успех. Бьёт ТОЛЬКО по доскам, трансформированным ПОСЛЕ создания (`create_component` строит геометрию в мировых координатах с identity-трансформацией — поэтому smoke на кубах у origin проходил). Соседние `place_tenon`/`carve_board2_slots` уже корректны — их НЕ трогать (`carve_board2_slots` остаётся без нового тестового покрытия — осознанно). Границы фикса: компенсация математически точна для ЛЮБОЙ аффинной трансформации (прототип приземляется ровно там, где нарисован), но координаты реза по-прежнему считаются в МИРОВЫХ осях от мирового bbox — на ПОВЁРНУТОЙ доске резьба останется axis-aligned (та же семантика, что у production-проверенного `place_tenon`); подтверждённый скоуп — ДОСКИ, СДВИНУТЫЕ после создания (fake-тест translation-only + live smoke шаг 21), rotated-доски не заявляются. Side effect (намеренный): резьба теперь живёт внутри вложенного instance (board → instance → nested groups) — меняется глубина структуры для `list_components`/UI-редактирования; для bbox/booleans/undo прозрачно; функциональная проверка — live smoke.
...
git commit -m "fix: dovetail/finger carve helpers compensate board transformation — joints land on translated boards"
```
...
- Produces: инвариант «`_reader`/`_writer` мутируются ТОЛЬКО под `self._lock`»: `get_connection()` больше НЕ коннектит (только создаёт/возвращает singleton под модульным `_get_connection_lock`); новые публичные методы `async ensure_connected()` (eager-connect для lifespan; `ConnectionError` при отказе) и `async aclose()` (закрытие с ожиданием in-flight запроса; используется `close_connection()`). Ленивый connect в `_send_once` (уже под замком) остаётся единственным авто-коннектором. Механика бага: `get_connection` health-check'ал и коннектил под модульным замком, а error-paths `_send_once` звали `disconnect()`, который после await-точки (`wait_closed`) безусловно нулил `_reader/_writer` — интерливинг терял свежую пару соседа (ложные `-32603 internal: reader is None`) или утекал сокетом (зомби-клиент на Ruby-стороне до ~2 ч SO_KEEPALIVE). Осознанные цены «чистого» варианта: (1) `aclose()`/`close_connection()` под `conn._lock` дожидается in-flight `send_command` (до TIMEOUT, дефолт 60 с) + `wait_closed` — приемлемо: lifespan-finally выполняется после остановки тулов, in-flight там маловероятен; cancel-in-flight НЕ вводим. (2) `close_connection()` дополнительно сериализуется `_get_connection_lock` с `get_connection()` (см. Step 3c). (3) Низкоуровневые `connect()`/`disconnect()` остаются публичными lock-free примитивами — инвариант держится контрактом в их докстрингах (Step 3d) и дисциплиной вызывающих.
...
Expected: новые/переписанные тесты красные (`AttributeError: ... no attribute 'ensure_connected'/'aclose'`; `test_get_connection_does_not_open_socket` падает AssertionError из патча — текущий get_connection коннектит). Исключение: `test_lifespan_degrades_when_eager_connect_fails` (1d) на старом коде НЕ красный — старый lifespan не зовёт `ensure_connected`, тест проходит vacuously; это regression-pin нового пути, red-фазы у него не ждать.
...
(d) Докстринги-контракты на существующие `connect()` / `disconnect()` — добавить в каждый по строке:
...
Это единственное место, где будущий вызывающий узнаёт об инварианте T-08 — без пометки прямой вызов вернёт исходную гонку.

(e) Попутная гигиена при переписывании тестов: обновить docstring'и затронутых тестов под двухфазную механику (cold-start race теперь описывает `get_connection()` + `ensure_connected()`, а не «get_connection коннектит»); проверить, существует ли тест «`send_command` поднимает `ConnectionError` при отказе TCP (ленивый connect)» — если нет, добавить рядом с новыми (источник ошибки для callers переехал из `get_connection` в `send_command`). В коде `ensure_connected`/`_send_once` оставить комментарий, ПОЧЕМУ health-check продублирован: `asyncio.Lock` нереентерабелен — общий «проверь-и-коннекть» хелпер под замком из-под замка не вызвать без дедлока.
...
Run: `uv run pytest tests/ -q`
Expected: `135 passed` (132 − 1 заменённый + 2 новых в test_connection + 1 в test_app). Если какой-то ещё тест мокал старое поведение `get_connection`-connects — поправить его мок по образцу шага 1b (двухфазный `get_connection` + `ensure_connected`).
...
Там же в docstring: строку с «step 22 verifies this» (про version handshake) обновить на «step 25 verifies this» — после ренумерации хвоста handshake-шаг становится 25-м, а 22-м — новый eval-syntax-error.
...
Дополнительно по ревью: сверить cleanup-список с ФАКТИЧЕСКИМИ живыми переменными smoke на момент шага 23 и включить уцелевшие оригиналы более ранних шагов (кандидаты: id1/id2 — операнды boolean'ов копируются, оригиналы живы; ID после chamfer/fillet) — существующий список оставлял мусор в модели. NB-комментарий перед undo-шагом переформулировать честно: `delete_component` сам undoable, поэтому undo откатывает ПОСЛЕДНИЙ delete (воскрешает последний удалённый ID), а не создание mortise_tenon.
